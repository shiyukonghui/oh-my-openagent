...
/**
 * DESIGN.md module — parse, validate, and format Google-compatible
 * design system files for prompt injection and workspace management.
 *
 * Ported from open-codesign's shared/design-md.ts, adapted for
 * oh-my-openagent's Bun runtime and codesign agent context.
 */
...
function warning(path: string, message: string): DesignMdFinding {
...
function validateColors(findings: DesignMdFinding[], value: unknown): void {
  ...
  for (const [name, rawColor] of Object.entries(value)) {
    if (
      typeof rawColor !== "string" ||
      !/^#[0-9a-fA-F]{3}(?:[0-9a-fA-F]{3})?$/.test(rawColor)
    ) {
      findings.push(
        error(`colors.${name}`, "Color tokens must be sRGB hex values (e.g. #1A1C1E)")
      )
    }
  }
}
...
function validateTypography(findings: DesignMdFinding[], value: unknown): void {
  ...
  for (const [tokenName, rawToken] of Object.entries(value)) {
    if (!isRecord(rawToken)) {
      findings.push(
        error(`typography.${tokenName}`, "Typography token must be an object")
      )
      continue
    }
    for (const key of Object.keys(rawToken)) {
      if (!TYPOGRAPHY_KEYS.has(key)) {
        findings.push(
          warning(
            `typography.${tokenName}.${key}`,
            `Unknown typography property "${key}"`
          )
        )
      }
    }
    ...
    if (
      ...
      findings.push(
        error(`typography.${tokenName}.fontWeight`, "fontWeight must be a number")
      )
    }
...
function validateScale(
  ...
  for (const [name, rawValue] of Object.entries(value)) {
    if (key === "spacing") {
      validateDimensionOrNumber(findings, rawValue, `${key}.${name}`)
    } else {
      validateDimension(findings, rawValue, `${key}.${name}`)
    }
  }
}
...
function validateComponents(findings: DesignMdFinding[], value: unknown): void {
  ...
  for (const [componentName, rawComponent] of Object.entries(value)) {
    ...
    for (const [property] of Object.entries(rawComponent)) {
      if (!COMPONENT_KEYS.has(property)) {
        findings.push(
          warning(
            `components.${componentName}.${property}`,
            `Unknown component property "${property}"`
          )
        )
      }
    }
...
function validateSections(
  ...
  for (const section of sections) {
    ...
    if (seen.has(canonical)) {
      findings.push(
        error(`body.${canonical}`, `Duplicate DESIGN.md section "${canonical}"`)
      )
      continue
    }
    seen.add(canonical)
    if (index < lastKnown) {
      findings.push(
        warning(`body.${canonical}`, `Section "${canonical}" appears out of order`)
      )
    }
...
export function validateDesignMd(raw: string): DesignMdFinding[] {
  ...
  if (
    "description" in doc.frontmatter &&
  ...
  if ("typography" in doc.frontmatter)
    validateTypography(findings, doc.frontmatter["typography"])
  if ("rounded" in doc.frontmatter)
    validateScale(findings, doc.frontmatter["rounded"], "rounded")
  if ("spacing" in doc.frontmatter)
    validateScale(findings, doc.frontmatter["spacing"], "spacing")
  if ("components" in doc.frontmatter)
    validateComponents(findings, doc.frontmatter["components"])
...
export function generateDesignMd(tokens: DesignMdTokens): string {
  const frontmatter: Record<string, unknown> = {
    version: tokens.version ?? "alpha",
    name: tokens.name,
  }
  if (tokens.description) frontmatter["description"] = tokens.description
  if (tokens.colors) frontmatter["colors"] = tokens.colors
  if (tokens.typography) frontmatter["typography"] = tokens.typography
  if (tokens.rounded) frontmatter["rounded"] = tokens.rounded
  if (tokens.spacing) frontmatter["spacing"] = tokens.spacing
  if (tokens.components) frontmatter["components"] = tokens.components
  ...
  return `---\n${yamlBody.trimEnd()}\n---\n\n## Overview\n\nA design system for ${tokens.name}.`
}
...
function orderedBodySections(doc: DesignMdDocument): DesignMdBodySection[] {
  ...
  for (const section of doc.bodySections) {
    const canonical = canonicalSectionHeading(section.heading)
    if (KNOWN_SECTION_INDEX.has(canonical)) {
      known.push({ ...section, heading: canonical })
    } else {
...
function formatBodySectionsForPrompt(doc: DesignMdDocument): string {
  if (doc.bodySections.length === 0) {
    if (doc.body.length <= BODY_PROMPT_MAX_CHARS) return doc.body
    return `${doc.body.slice(0, BODY_PROMPT_MAX_CHARS).trimEnd()}\n\n[DESIGN.md body truncated to ${BODY_PROMPT_MAX_CHARS} chars]`
  }
...
export function formatProjectDesignSystemContext(raw: string): string {
  try {
    const formatted = formatDesignMdForPrompt(raw)
    return `# Project Design System (DESIGN.md)\n\n${formatted}`
  } catch {
    return `# Project Design System (DESIGN.md)\n\nDesign system present but could not be formatted.`
  }
}

export function formatProjectInstructionsContext(raw: string): string {
...
export function formatUntrustedContext(
  ...
  return [
    `<untrusted_scanned_content id="${id}">`,
    description,
    "",
    content,
